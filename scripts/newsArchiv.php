<?php
$news = '';

$cond = "n.archiv={p} AND n.typ={p}";
$paramsArr[] = 1;
$paramsArr[] = 1;

$pos = 0;

$fn = "newsfilter1";
  
if(isset($_GET['filter']) && $_GET['filter'] == 'reset' && isset($_SESSION[$fn])) { unset($_SESSION[$fn]); }
if(isset($_GET['pos'])) { $_SESSION[$fn]['pos'] = (is_numeric($_GET['pos']))?$_GET['pos']:0; }
if(isset($_SESSION[$fn]['pos'])) { $pos = (int)$_SESSION[$fn]['pos']; }
if($pos < 0) { $pos = 0; }

$sql = "
SELECT
  COUNT(n.ID) AS anz
  
FROM
  news n
  
WHERE
  {$cond}
";
$qry = $DB_LINK -> query($sql, $paramsArr);
$res = $qry -> fetch_object();
if($res -> anz == 0) {
 	$news = "<p>Es gibt keine archivierten Neuigkeiten.</p>";

} else {
	
  $pagination = $showPage -> getPagenavi("newsArchiv", $res -> anz, $pos);
  $news .= $pagination;
  $sql = "
  SELECT
    n.ID, n.titel, n.teaser, n.text
 
  FROM
    news n
    
  WHERE
    {$cond}
  
  ORDER BY
    n.datum DESC

  LIMIT
    {p}, {p}
  ";

  $paramsArr[] = $pos;
  $paramsArr[] = $showPage -> config['lists']['entriesPerPage'];

  $qry = $DB_LINK -> query($sql, $paramsArr);
  while($res = $qry -> fetch_object()) {

    $news .= "<div class=\"startnews group\"><h3>{$res-> titel}</h3>{$res -> teaser}";
    if($res -> text != "") { $news .= "<p><a href=\"newsDetails-{$res -> ID}.html\">weitere Informationen</a></p>"; }
    $news .= "</div>\n";

  }
  $news .= $pagination;
}

$platzhalter['news'] = $news;
?>