...
if (count($jpArr['gruppen'][$gruppe]) == 0) {
    ...
    foreach ($jpArr['gruppen'][$gruppe] AS $val) {
        $gt = ($val == $typ) ? "<strong>{$jpArr['typen'][$val]['kurz']}</strong>" : "<a href=\"jp-{$gruppe}-{$val}.html\">{$jpArr['typen'][$val]['kurz']}</a>";
        $gruppen .= "<li>{$gt}</li>\n";
    }
    $gruppen .= "</ul>\n</div>";
...
for ($i = $maxJahr; $i >= $minJahr; $i--) {
    if ($i == $currJahr) {
        ...
        $jArr[] = "<a href=\"jp-{$gruppe}-{$typ}-{$i}.html\">{$i}</a>";
...
if ($res->anz == 0) {
  ...
  ";
    ...
    while ($res = $qry->fetch_object()) {
        ...
        $filename = 'event.ics';
        $export = ($res->export == 1) ? "<a href=\"/calendar/{$res->ID}/{$filename}\" title=\"In Kalender übernehmen\"><img src=\"/images/calendar_add.png\" alt=\"\" /></a>" : '';

        $liste .= "<tr{$alt}><td>{$datum}</td>\n<td><a href=\"anlass-{$gruppe}-{$typ}-{$currJahr}-{$res->ID}.html\">{$res->titel}</a></td>\n<td>{$res->ort}</td>\n<td>{$export}</td></tr>\n";
    }
    $liste .= "</tbody>\n</table>";
