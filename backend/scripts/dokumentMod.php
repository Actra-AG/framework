...
if($showPage -> checkUG('aktiv')) {
	...
	if($objekt == 'anlass') {
	  ...
	  WHERE
  	  ID='{p}' AND (registered_by='{p}' OR '{p}'=1)
  	";
  	$qry = $DB_LINK -> query($sql, array($objektID, $myID, $showPage -> userData -> admin));
	...
	$sql = "SELECT titel FROM dokumente WHERE objekt='{p}' AND objektID='{p}' AND ID='{p}'";
    ...
    if(isset($_FILES['doc']) && $_FILES['doc']['name'] != '') {
      ...
      $sql = "SELECT extension FROM dateiformate WHERE mimetype='{p}' AND FIND_IN_SET('dokumente', arten)!=0";
  	...
  	if(isset($_POST['titel'])) {	$datenArr['titel'] = $_POST['titel']; }
