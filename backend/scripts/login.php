...
if(isset($_GET['send'])) {
  ...
  if(count($fehlerArr) == 0) {
    ...
    if($qry -> num_rows() != 1) {
      ...
      if($personData -> confirmed == '0000-00-00 00:00:00') {
      ...
      } elseif($personData -> passwort != $db_passwort) {
        $fehlerArr[] = 'Sie haben ungültige Zugangsdaten eingegeben.';
        ...
        $DB_LINK -> query("UPDATE benutzer SET lastlogin=NOW(), wronglogin=0, visits=visits+1 WHERE ID={p}", array($personData -> ID));
        $DB_LINK -> query("INSERT INTO visits SET benutzerID='{p}', sessionID='{p}', ip='{p}'", array($personData -> ID, session_id(), $ip));
