...
if($showPage -> checkUG('redaktor')) {
	...
	if(isset($_GET['del'])) {
		$DB_LINK -> query("DELETE FROM alben WHERE ID='{p}'", array($_GET['del']));
    ...
    $qry = $DB_LINK -> query("SELECT pos FROM alben WHERE ID='{p}'", array($albumID));
    $res = $qry -> fetch_assoc();
    $newPos = $res['pos'];
    if($action == 'up') {
    	$newPos = $res['pos'] - 1.5;
    ...
    $newPos = str_replace(",", ".", $newPos);
   	$DB_LINK -> query("UPDATE alben SET pos='{p}' WHERE ID='{p}'", array($newPos, $albumID));
    ...
    $i = 0;
  	$qry = $DB_LINK -> query("SELECT ID FROM alben ORDER BY pos");
 	  while($res = $qry -> fetch_assoc($qry)) {
   	  $i++;
 	    $albumID = $res['ID'];
   	  $DB_LINK -> query("UPDATE alben SET pos={$i} WHERE ID='{p}'", array($albumID));
 	  }
	}
  ...
  if($anz == 0) {
    ...
    while($res = $qry -> fetch_assoc()) {
    	...
    	$pArr = array();
    	if($i != 1) { $pArr[] = "<li class=\"oben\"><a href=\"alben.html?up={$res['ID']}\">nach&nbsp;oben</a></li>"; }
    	if($i != $anz) { $pArr[] = "<li class=\"unten\"><a href=\"alben.html?down={$res['ID']}\">nach&nbsp;unten</a></li>"; }

      $liste .= "<tr>\n<td>{$res['titel']}</td>\n<td>{$res['fotos']} [<a href=\"fotos-{$res['ID']}.html\">verwalten</a>]</td>\n<td class=\"pos\">";
      if(count($pArr) == 0) {
      	$liste .= "&nbsp;";
      } else {
      	$liste .= "<ul>";
      ...
      $liste .= "</td>\n<td class=\"aktion\">\n<ul>\n<li><a href=\"albumMod-{$res['ID']}.html\" class=\"edit\">bearbeiten</a></li>\n<li><a href=\"alben.html?del={$res['ID']}\" class=\"delete\">löschen</a></li>\n</ul>\n</td>\n</tr>\n";
    }
    $liste .= "</tbody>\n</table></div>";
    $liste .= $pagination;
  }
