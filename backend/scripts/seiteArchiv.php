...
if($showPage -> checkUG('redaktor') && isset($showPage -> arrVars[2])) {
  ...
  $ortname = $oArr[$ort]['name'];
	...
	$sql = "SELECT s.ID, DATE_FORMAT(s.datum, '%d.%m.%Y %T') AS datum, CONCAT(b.vorname, ' ', b.nachname) AS name FROM seiteninhalte s LEFT JOIN benutzer b ON s.benutzerID=b.ID WHERE s.ort='{p}' AND s.seite='{p}' ORDER BY s.datum DESC";
	...
	if($qry -> num_rows() == 0) {
		...
		while($res = $qry -> fetch_assoc()) {
			$liste .= "<li><a href=\"archiv-seite-{$res['ID']}.html\">{$res['datum']}</a> [{$res['name']}]</li>";
		}
