...
//  }
	...
	if (isset($_GET['remove'])) {
		$DB_LINK->query("DELETE FROM jahresprogramm WHERE ID=? AND (registered_by=? OR {$showPage->userData->admin}=1)", array($_GET['remove'], $myID));
	}
	...
	$cond = "WHERE (j.registered_by=? OR {$showPage->userData->admin}=1)";
	$paramsArr[] = $myID;
