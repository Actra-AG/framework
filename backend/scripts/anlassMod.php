...
if ($showPage->checkUG('aktiv')) {
	...
	";
    $qry = $DB_LINK->query($sql, array($ID, $myID, $showPage->userData->admin));
    if ($qry->num_rows() == 1) {
        ...
        if (count($fehlerArr) == 0) {
            ...
            if ($ID == 0) {
                ...
                if ($showPage->checkUG('admin') == 1) {
                    $DB_LINK->query("UPDATE jahresprogramm SET confirmed=NOW() WHERE ID='{p}'", array($ID));
...
//        FormMailer::sendMail("info@actra.ch", $toName, $from, $fromName, $subject, $text, "no", "txt");
            ...
            $DB_LINK->query("UPDATE jahresprogramm SET lastmod=NOW() WHERE ID='{p}'", array($ID));
