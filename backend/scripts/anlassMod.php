...
class anlassMod extends pageClass
{
	...
	";
			...
			if ($qry->rowCount() == 1) {
				...
				$ID = 0;
				$ac = 'add';
				$this->showPage->pageArr['platzhalter']['title'] = 'Anlass hinzufügen';
			}
			...
			if (isset($_GET['send'])) {
				...
				} else if (!preg_match("/[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{4}/", $_POST['datumVon'])) {
				...
				} else if (!preg_match("/[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{4}/", $_POST['datumBis'])) {
				...
				if (count($fehlerArr) == 0) {
					...
					if ($ID == 0) {
						...
						$text = "Grüezi\n\nDer Anlass {$datenArr['titel']} wurde bei {$_SERVER['SERVER_NAME']} durch den Benutzer {$this->showPage->userData->vorname} {$this->showPage->userData->nachname} geändert.\n\nFreundliche Grüsse\n\nBezirksschützenverband Bülach";
