   ...
   ID=?";
	...
	if (isset($_GET['del'])) {
		...
		if ($qry->rowCount() == 1) {
			...
			$DB_LINK->query("DELETE FROM fotos WHERE ID=?", array($_GET['del']));
	...
	if (isset($_GET['up']) || isset($_GET['down'])) {
		...
		$qry = $DB_LINK->query("SELECT pos FROM fotos WHERE ID=?", array($fotoID));
		$res = $qry->fetch(PDO::FETCH_ASSOC);
		$newPos = $res['pos'];
		if ($action == 'up') {
			$newPos = $res['pos'] - 1.5;
		...
		$DB_LINK->query("UPDATE fotos SET pos=? WHERE ID=?", array($newPos, $fotoID));

		$i = 0;
		$qry = $DB_LINK->query("SELECT ID FROM fotos WHERE albumID=? ORDER BY pos", array($ID));
		while ($res = $qry->fetch(PDO::FETCH_ASSOC)) {
			$i++;
			$fotoID = $res['ID'];
			$DB_LINK->query("UPDATE fotos SET pos={$i} WHERE ID=?", array($fotoID));
		}
	}
