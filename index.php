...
if($config['useDynDir']) {
  require_once($_SERVER['DOCUMENT_ROOT'].'/dynDir.php');
}
...
if($requestHandler -> reqType == 'file') {
  ...
  if($config['useDB'] == 1) {
  	switch($config['DB']['engine']) {
		  case 'mysqli': $DB_LINK = MySQLiDB::getInstance(); break;
		  case 'mysql': $DB_LINK = MySQLDB::getInstance(); break;
	  }
  }
  ...
  if($requestHandler -> reqType == 'service' && $requestHandler -> serviceName) {
    # load service
  	...
  	require_once($_SERVER['DOCUMENT_ROOT'].'/services/'.$requestHandler -> serviceName.'.php');
