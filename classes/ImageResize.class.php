...
class imageresize {
  ...
  public function resize_image($tmp_name, $ext, $new_img_loc='', $new_img_name='', $thumb_w=0, $thumb_h=0, $new_img_q=90, $cut=0, $createOrig = 1) {
    ...
    if(!file_exists($orig_img)) {
      ...
      if($orig_w < $thumb_w) { $thumb_w = $orig_w; }
      if($orig_h < $thumb_h) { $thumb_h = $orig_h; }
      ...
      switch($ext) {
        case 'gif': $orig_image = imagecreatefromgif($orig_img); $thumb_image = imagecreate($thumb_w, $thumb_h); break;
        case 'jpg': $orig_image = imagecreatefromjpeg($orig_img); $thumb_image = imagecreatetruecolor($thumb_w, $thumb_h); break;
        case 'png': $orig_image = imagecreatefrompng($orig_img); $thumb_image = imagecreatetruecolor($thumb_w, $thumb_h); break;
      }
 		  if(!$orig_image) { return false; }
      ...
      switch($ext) {
        case 'gif': imagegif($thumb_image, $newimg); break;
        case 'jpg': imagejpeg($thumb_image, $newimg, $new_img_q); break;
        case 'png': imagepng($thumb_image, $newimg); break;
      }
