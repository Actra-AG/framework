...
if(isset($reqArr[4])) {
  ...
  if($code != $reqArr[3]) {
    ...
    $sql = "SELECT d.type, f.extension, d.dateiname FROM dokumente d INNER JOIN dateiformate f ON d.type=f.mimetype WHERE d.ID='{p}'";
    ...
    if($qry -> num_rows() == 1) {
        ...
        ErrorHandler::display_error(404);
      	...
      	$DB_LINK -> query("UPDATE dokumente SET views=views+1 WHERE ID={p}", array($ID));
      	$ok = 1;
      }
    }
  }
